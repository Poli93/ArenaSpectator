...
local function CreateFrameForPlayer(p)
    ...
    CombatFeedbackFrame:SetPoint("CENTER", 0, 0)
    f.CombatFeedbackText = CombatFeedbackFrame:CreateFontString(nil, "OVERLAY")
    f.CombatFeedbackText:SetPoint("CENTER", 80, 6)
...
local function ForceUpdate()
    SendChatMessage(".spec reset", "GUILD");
end
...
function SetEndTime(newTime)
    ...
    TimeFrame:SetScript("OnUpdate", function(self, elapsed)
        ...
        if tournamentMode then
            local runtime = (12 * 60) - self.time
            local nextevent = 0
            local amount = 0
            if runtime < (5 * 60) then
                nextevent = 5 * 60
                amount = 10
            elseif runtime < (7 * 60) then
                nextevent = 7 * 60
                amount = 20
            elseif runtime < (9 * 60) then
                nextevent = 9 * 60
                amount = 30
            elseif runtime < (11 * 60) then
                nextevent = 11 * 60
                amount = 40
            end

            if nextevent then
                local seconds = math.floor((nextevent - runtime) % 60)
                if seconds < 10 then seconds = "0" .. seconds end
                local minutes = math.floor((nextevent - runtime) / 60)
                damageBuffFrame.text:SetText("+" .. amount .. "% in " .. minutes .. ":" .. seconds)
            else
                damageBuffFrame.text:SetText("")
            end
        end
    end)
end
...
local function AddCooldown(target, spell, cooldown)
    if (GetTableSize(ATPlayers) == 8) then
        for _, barname in pairs(ALLBARS) do
...
local function ACPEC_ShowOutdatedAddonVersion()
    StaticPopupDialogs["ARENASPECTATOR_OUTDATED"] = {
        text = "Your version of Arena-Spectator UI is outdated. Please visit http://arena-tournament.com to download last version.",
        button1 = "Ok",
        whileDead = true,
        timeout = 0,
        hideOnEscape = false,
...
local function EventHandler(self, event, arg1, arg2, arg3, arg4)
    if (event == "PLAYER_LOGIN") then
        SendChatMessage(".spectator version " .. GetAddOnMetadata("ArenaSpectator", "Version") or "0", "GUILD");
    end
    ...
    if (event == "CHAT_MSG_ADDON") and (arg1 == "ARENASPEC") and (arg3 == "WHISPER") and (arg4 == "") and (arg2 == "OUTDATED") then
...
function ASPEC_try_spectate(self, name, unit, server)
    SendChatMessage(".spec spectate " .. name, "GUILD");
end
...
function setupScoreboardFrame()
    ...
    logoFrame = CreateFrame("frame", nil, WorldFrame)
    logoFrame:SetWidth(200)
    logoFrame:SetHeight(100)
    logoFrame:SetPoint("TOP", 0, 0)
    logoFrame:SetBackdrop({bgFile = "Interface\\AddOns\\ArenaSpectator\\atLogo"})
    logoFrame:Hide()
...
local function init()
    ...
    damageBuffFrame = CreateFrame("Frame", nil, WorldFrame)
    damageBuffFrame.text = damageBuffFrame:CreateFontString()
    damageBuffFrame.text:SetFont(STANDARD_TEXT_FONT, 14, "OUTLINE")
    damageBuffFrame.text:SetPoint("TOP", WorldFrame, "TOP", 0, -105)
    damageBuffFrame.text:SetText("")
    damageBuffFrame:Show()
